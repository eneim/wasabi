...
private val parsePrivateUse =
  entireSliceOf(
    take('x') then parseRepeatedly(take('-') concat takeExactlyWhile(1, 8, AlphaNumericChars), 1)
  )
...
private val parseExtension: Parser<Extension> = parseSingleton.flatMap { prefix ->
  take('-').andThenTake(parseSeparatedList(takeExactlyWhile(2, 8, AlphaNumericChars), take('-'), 1))
    .map { parts ->
      Extension(prefix, parts.map { it.toString() })
    }
}

private val parseVariant =
  (takeExactlyWhile(5, 8, AlphaNumericChars) or (take(Digit) concat takeExactlyWhile(
    3, 3, AlphaNumericChars
  )))
...
private val parseExtlang = entireSliceOf(
  takeExactlyWhile(3, 3, Alpha) then parseRepeatedly(
    take('-') then takeExactlyWhile(3, 3, Alpha), 0, 2
  )
)
