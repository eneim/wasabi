...
private val parseMediaDesc =
  takeUntil(min = 1, oneOf = parameterFollowSet + '"').map { it.toString() }
...
private val parseExtRelType: Parser<String> =
  trace("ext-rel-type", parseUri(parameterFollowSet + '"'))

private val parseRelationType = trace("relation-type", parseExtRelType or parseRegRelType)

private val parseRelationTypes: Parser<List<String>> = trace(
  "relation-types",
  takeFirst(
...
private fun <T : Any> parseParameter(
  name: String,
  value: Parser<T>
): Parser<T> =
...
private val parseAnchorParam: Parser<Parameter.Anchor> =
  parseParameter(
    Parameter.Anchor.NAME, quoted(parseUriReference(setOf('"'))).map(Parameter::Anchor)
  )
...
private val parseMediaParam: Parser<Parameter.Media> =
  parseParameter(
    Parameter.Media.NAME, (quoted(parseMediaDesc) or parseMediaDesc).map(Parameter::Media)
  )
...
private val parseTypeParam: Parser<Parameter.Type> =
  parseParameter(
    Parameter.Type.NAME, (parseQuotedMediaType or parseMediaType).map { Parameter.Type(it) })
...
private val parseLinkValue =
  parseUriReference(setOf('>')).surroundedBy(takeWithLws('<'), takeWithLws('>'))
