...
fun <T, U> Parser<T>.flatMap(f: (T) -> Parser<U>): Parser<U> = trace("flatMap()") { input ->
  when (val result = this(input)) {
    is Result.Success -> f(result.value)(result.rest)
...
fun <T, U> Parser<T>.map(f: (T) -> U): Parser<U> = { input ->
  when (val result = this(input)) {
    is Result.Success -> Result.Success(f(result.value), result.rest)
    is Result.Error -> result
  }
}
...
fun <T> Parser<T>.andThenIgnore(next: Parser<*>): Parser<T> = trace("andThenIgnore()") { input ->
  when (val result = this(input)) {
    is Result.Success -> when (val nextResult = next(result.rest)) {
      is Result.Success -> Result.Success(result.value, nextResult.rest)
...
infix fun Parser<StringSlice?>.concat(next: Parser<StringSlice?>): Parser<StringSlice> =
  trace("concat()") { input ->
    when (val first = this(input)) {
      is Result.Success -> when (val second = next(first.rest)) {
        is Result.Success -> {
          val firstValue = first.value ?: ""
          val secondValue = second.value ?: ""
          Result.Success(input.subSlice(0, firstValue.length + secondValue.length), second.rest)
        }
...
fun <T, U> Parser<T>.andThenTake(trailer: Parser<U>): Parser<U> = trace("andThenTake()") { input ->
  when (val result = this(input)) {
    is Result.Success -> when (val trailerResult = trailer(result.rest)) {
      is Result.Success -> trailerResult
...
fun <T> Parser<T>.surroundedBy(
  prefix: Parser<*>,
  suffix: Parser<*>
): Parser<T> = trace("surroundedBy()") { input ->
      ...
      when (val result = this(prefixResult.rest)) {
        ...
        is Result.Success -> {
          when (val suffixResult = suffix(result.rest)) {
            ...
            is Result.Success -> Result.Success(result.value, suffixResult.rest)
          }
        }
      }
    }
...
fun <T : Any> Parser<T>.optional(): Parser<T?> = trace("optional()") { input ->
  when (val result = this(input)) {
    ...
    is Result.Error -> Result.Success(null, input)
  }
}

fun <A, B> parseSeparated(
  a: Parser<A>,
  separator: Parser<*>,
  b: Parser<B>
): Parser<Pair<A, B>> =
...
fun <T> parseSeparatedList(
  parser: Parser<T>,
  separator: Parser<*>,
  min: Int = 0,
  max: Int = -1
): Parser<List<T>> = trace("parseSeparatedList(min=$min, max=$max)") { input ->
  when (max) {
    in 0 until min -> Result.Error("Min ($min) can't be larger than max ($max)!", input)
    0 -> Result.Success(emptyList(), input)
    else -> {
      when (val first = parser(input)) {
        ...
        is Result.Success -> {
          ...
          if (output.size < min) Result.Error(
            "only matched ${output.size} times, $min required!", input
          )
          else Result.Success(output.toList(), rest)
        }
      }
    }
  }
...
fun <T : Any> takeFirst(vararg parsers: Parser<T>): Parser<T> =
  trace("takeFirst(#parsers=${parsers.size})") { input ->
    parsers.asSequence().map { it(input) }.firstOrNull { it is Result.Success }
      ?: Result.Error("No parser worked!", input)
  }

fun <T> parseRepeatedly(
  parser: Parser<T>,
  min: Int = 0,
  max: Int = -1
): Parser<List<T>> = trace("parseRepeatedly(min=$min, max=$max, parser=...)") { input ->
  when (max) {
    in 0 until min -> Result.Error("Min ($min) can't be larger than max ($max)!", input)
    0 -> Result.Success(emptyList(), input)
    else -> {
      ...
      else Result.Success(output, rest)
    }
  }
}

fun takeWhile(predicate: (Char) -> Boolean): Parser<StringSlice> =
  takeWhile(min = 0, max = -1, predicate = predicate)

private fun takeWhilePredicate(
  min: Int = 0,
  max: Int = -1,
  predicate: (Char) -> Boolean
): Parser<StringSlice> = { input ->
  var len = 0
  var left = max
  ...
  if (len >= min) Result.Success(input.subSlice(0, len), input.subSlice(len))
...
fun takeWhile(
  ...
  trace(
    "takeWhile(min=$min, max=$max, predicate=$predicate)", takeWhilePredicate(min, max, predicate)
  )
...
fun takeWhile(
  ...
  trace(
    "takeWhile(min=$min, max=$max, oneOf=${forTrace(oneOf)})",
    takeWhilePredicate(min, max, oneOf::contains)
  )
...
fun takeWhile(
  ...
  trace(
    "takeWhile(min=$min, max=$max, oneOf=${forTrace(c)})", takeWhilePredicate(min, max) { it == c })
...
fun takeUntil(
  ...
  trace(
    "takeUntil(min=$min, max=$max, oneOf=${forTrace(oneOf)})", takeWhile(min, max) { it !in oneOf })

private fun takeExactlyWhilePredicate(
  min: Int,
  max: Int,
  predicate: (Char) -> Boolean
): Parser<StringSlice> = { input ->
  ...
  when {
    ...
    else -> Result.Success(input.subSlice(0, len), input.subSlice(len))
  }
}

fun takeExactlyWhile(
  min: Int,
  max: Int,
  oneOf: Set<Char>
) =
  trace(
    "takeExactlyWhile(min=$min, max=$max, oneOf=${forTrace(oneOf)})",
    takeExactlyWhilePredicate(min, max, oneOf::contains)
  )
...
private fun takeOneChar(predicate: (Char) -> Boolean): Parser<StringSlice> = { input ->
  if (input.isEmpty()) Result.Error("no input left!", input)
    ...
    val c = input[0]
    if (predicate(c)) Result.Success(input.subSlice(0, 1), input.subSlice(1))
...
fun takeString(s: CharSequence): Parser<StringSlice> =
  trace("take(s=\"${forTrace(s, "\"")}\")") { input ->
    if (input.startsWith(s)) Result.Success(input.subSlice(0, s.length), input.subSlice(s.length))
    else Result.Error("$s was not recognized!", input)
  }

fun takeString(strings: Set<CharSequence>): Parser<StringSlice> =
  trace("take(strings=${forTrace(strings)})") { input ->
    val firstMatching = strings.firstOrNull { input.startsWith(it) }
    if (firstMatching == null) Result.Error("No string matched!", input)
    else Result.Success(
      input.subSlice(0, firstMatching.length), input.subSlice(firstMatching.length)
    )
  }
...
fun entireSliceOf(parser: Parser<*>): Parser<StringSlice> = trace("entireSliceOf()") { input ->
  when (val result = parser(input)) {
    is Result.Success -> Result.Success(
      input.subSlice(0, input.length - result.rest.length), result.rest
    )
    is Result.Error -> result
  }
}
